export default {
    ...
    actions: {
        async fetch_contragent_t(ctx) {
            const res = await fetch(ctx.rootState.hostname + '/OquvMarkazClient/getOquvMarkazContragentPagination?page=0&size=100');
    ...
    mutations: {
        update_contragent_t(state, data) {
            console.log(data)
            state.contragent_t.rows = data.items_list;
        },

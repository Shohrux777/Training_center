  ...
  methods: {
    ...
    async submit(){
        ...
        try{
          ...
          console.log(this.md)
            const response = await fetch(this.$store.state.hostname + '/OquvMarkazAuth/checkLoginPassoword?login=' + this.login + '&password=' + this.md)
          ...
          if(response.status == '200' || response.status == '201'){
            ...
            localStorage.CompId = 1;
            localStorage.UserId = data.oquvMarkazUserid;
            localStorage.UserType = data.user_type;
            await this.fetchUserId(data.oquvMarkazUserid);
            if(data.user_type == 2){
              this.$router.push('/mentor_page/'+ data.oquvMarkazUserid)
              return
            }
    ...
    async fetchUserId(id){
      try{
        const res = await fetch(this.$store.state.hostname + '/OquvMarkazUser/' + id);

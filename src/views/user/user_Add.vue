    ...
    <div class="user_add p-5">
      <form @submit.prevent="submit">
        ...
        <div class="mt-4 row_w">
          ...
          <div class="col_5 px-2" style="position: relative;">
              <erpSelect
                :options="allPosition.rows"  
                @select="selectOptionPosition"
                :selected="position_name"
                :label="$t('company')"

              />
              <small v-if="$v.position_name.$dirty && position_id == null" class="invalid-text mt-0 ml-2" >
                  {{$t('select_item')}}
                </small>
          </div>
        </div>  
        ...
        if(this.$v.$invalid)
          ...
          const requestOptions = {
              ...
              body: JSON.stringify({
                ...
                "oquvMarkazDepartmentid": this.dept_id,
                "oquvMarkazPositionid": this.position_id,
                "note": this.note,
                "id" : this.id,
              })
            };
            try{
              this.loading = true;
              const response = await fetch(this.$store.state.hostname + "/OquvMarkazUser", requestOptions);

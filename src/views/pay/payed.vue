<template>
  <div class="addProductQtyPayed" >
    ...
    <div v-else class="p-4 mainPayedSection">
            ...
            <tbody>
              <tr v-for="(row,rowIndex) in clientPaymentListArr" :key="rowIndex" >
                ...
                <td class="text-center">
                  <span>
                    <i class="fas fa-trash-restore-alt delIcon mask waves-effect m-0" :class="{'applied': disable}" v-on:click="deleteRow(row,rowIndex)" :data-row="rowIndex"></i>
                  </span>
                </td>
      ...
      <div class="acceptBoxPayed px-3 py-2 text-center">
        ...
        <div style="margin-top:15px;" class="d-flex justify-content-end">
          <mdb-btn color="danger" style="font-size:11px; padding: 6px 0; width:120px;"  @click="paymentDeleteList" class="m-0  mt-1 mr-2" > {{$t('delete')}}</mdb-btn>
          <mdb-btn color="warning" style="font-size:11px; padding: 6px 0; width:120px;"  @click="close" class="m-0  mt-1 mr-2" > {{$t('cancel')}}</mdb-btn>
    ...
    <modal-train  :show="payment_status" headerbackColor="white"  titlecolor="black" :title="$t('delete')" 
      @close="payment_status = false" width="700px">
        <template v-slot:body>
          <div class="p-2">
            <table class="myTablePayedcheckList ">
              ...
              <tbody>
                <tr v-for="(row,index) in paymentListGroupId" :key="index" >
                  <td> <span >{{index+1}}</span> </td>
                  <td> <span >{{row.client.fio}}</span> </td>
                  <td> <span >{{row.group.name}}</span> </td>
                  <td> <span class="text-danger">{{row.created_date_time.slice(0,19)}}</span></td>
                
                  <td class="text-center">
                    <span>
                      <i class="fas fa-trash-restore-alt delIcon mask waves-effect m-0" :class="{'applied': disable}" v-on:click="deleteRowPayment(row,index)" :data-row="index"></i>
                    </span>
                  </td>
                  <!-- <td> <span >{{row.lessons_cout}}</span> </td> -->
...
export default {
  ...
  methods: {
    ...
    // })
      ...
      await this.getPaymentClientIDAndGroupId();
      ...
      if(this.clientPaymentList.length>0){
        ...
        for(let i=0;i<this.clientPaymentList.length; i++){
          ...
          let testObject = {
            ...
            id: this.clientPaymentList[i].id
          }
    ...
    //<==== get payment list from base ====>
    async getPaymentClientIDAndGroupId(){
      try{
        this.info_loading = true; 
        const response = await fetch(this.$store.state.hostname + "/OquvMarkazFanAndGroupPayment/getPaginationByClientIdAndGroupId?page=0&size=100&client_id=" + this.user.user_id + "&group_id=" + this.user.group_id);
        const data = await response.json();
        console.log('paymentListGroupId')
        console.log(data)
        if(data){
          this.paymentListGroupId = data.items_list;
        }
        else{
          this.modal_info = this.$i18n.t('network_ne_connect');;
          this.modal_status = true;
    ...
    async deleteRow(list, index){
      ...
      const response = await fetch(this.$store.state.hostname + "/OquvMarkazGruppagaTolovs/" + list.id, requestOptions);
      const data = await response.text();

      if(response.status == 201 || response.status == 200)
      {
        this.$refs.message.success('Successfully_removed')
        this.clientPaymentListArr.splice(index,1);
     ...
     async deleteRowPayment(item, i){
      console.log(item, i)
      const requestOptions = {
        method : "delete",
      };
      const response = await fetch(this.$store.state.hostname + "/OquvMarkazFanAndGroupPayment/" + item.id, requestOptions);
      const data = await response.text();

      if(response.status == 201 || response.status == 200)
      {
        this.$refs.message.success('Successfully_removed')
        this.paymentListGroupId.splice(i,1);

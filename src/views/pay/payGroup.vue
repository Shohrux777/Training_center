  ...
  <div>
    ...
    <div class="px-3">
      ...
      <table class="myTablePayedInfoPayed ">
        ...
        <tbody>
          <tr v-for="(row,rowIndex) in clientList" :key="rowIndex" :class="{'bg_dark text-white' : row.lessons_cout < 0,'bg_danger text-white' : row.lessons_cout == 0, 'bg_warning text-white' : row.lessons_cout >=1 && row.lessons_cout <3}">
...
export default {
  ...
  async created()
  {
    ...
    if(this.id > 0)
    {
      ...
      this.ceshList = this.group_client_list;
      this.clientList = this.group_client_list;
      try{
  ...
  methods: {
    ...
    async activ(option){
      ...
      this.$refs.payed.changingEnter(1, option.group.fan.price, {name: option.oquvchi.fio, family: option.oquvchi.passport_number_str, user_id:option.oquvchi.id, uqtuvchi_id:this.group_data.user.id,
    ...
    searchClick(){
      ...
      if(this.search != ''){
        ...
        for(let i=0; i<this.clientList.length;i++){
          if(this.clientList[i].oquvchi.fio.toLowerCase().includes(this.search.toLowerCase()) || this.clientList[i].oquvchi.passport_number_str.toLowerCase().includes(this.search.toLowerCase())){

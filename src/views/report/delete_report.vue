            ...
            <div class="col-sm-4 col-md-3 col-lg-3">
              <div class="d-flex justify-content-start align-items-center pt-2">
                <erpSelectReg
                  ...
                  :label="$t('company')"
                />
                <small style="position:absolute; top:-9px; left:18px; color:#747474; font-size:11.5px;">{{$t('trash_type')}}</small>
              </div>
            </div>
  ...
  methods: {
    ...
    async selectOptionReg(option){
      ...
      if(option.id == 0){
        ...
        try{
        const res = await fetch(this.$store.state.hostname + '/OquvMarkazPupilGroupDetailInfo/getPaginationUdalitQilinganlarListSearcByNote?page=0&size=1001&note=' + option.name);
    ...
    async selectMonth(date){
      ...
      this.loading = true;
      try{
        const res = await fetch(this.$store.state.hostname + '/OquvMarkazPupilGroupDetailInfo/getPaginationUdalitQilinganlarList?page=0&size=250');
        const data = await res.json();
        console.log(data)
        if(res.status == 200 || res.status == 201){
          console.log('das')

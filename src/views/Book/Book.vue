...
export default {
  ...
  methods: {
    ...
    ...mapMutations(['Book_delete_row']),
  
      for_edit(edit_data)
      {
        this.$router.push("/BookAdd/" + edit_data.id);
      },
      async for_delete(del_data,index)
      {
         const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/OquvMarkazBooks/" + del_data.id, requestOptions);

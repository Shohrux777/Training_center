<template>
  <div>
    ...
    <div class="Book_UnitsAdd px-5 pt-4">
      <form @submit.prevent="submit">
        <div class="row_w">
          ...
          <div class="col_5 px-2">
            <input-icon  v-model="name"  validate error="wrong" success="right"  icon="fas fa-book"/>
          ...
          <div class="col_3">
            <p class="p-0 m-0 mt-2" style="font-size: 14px;">{{$t('Book')}}</p>
          </div>
          <div class="col_5 px-2" style="position: relative;">
              ...
              />
              <small v-if="$v.book_name.$dirty && book_id == null" class="invalid-text mt-0 ml-2" >
                  {{$t('select_item')}}
                </small>
          </div>
        </div> 
...
export default {
  ...
  validations: {
    ...
    async created()
    {
      ...
      if(this.id > 0)
      {
        const res = await fetch(this.$store.state.hostname + '/OquvMarkazBookUnits/' + this.id);
    ...
    save_data :  async function(){
        if(this.$v.$invalid)
          ...
          const requestOptions = {
            ...
            try{
              this.loading = true;
              const response = await fetch(this.$store.state.hostname + "/OquvMarkazBookUnits", requestOptions);
    ...
    async submit_continue()
      {
        ...
        {
          if(this.id != 0){
            this.$router.push('/Book_UnitsAdd/0')
            this.cls_wnd();
          }
          else{
            this.cls_wnd();

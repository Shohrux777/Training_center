<template>
  <div>
    ...
    <div class="company_add px-5 pt-4">
...
export default {
  ...
  validations: {
    ...
    async created()
    {
      ...
      if(this.id > 0)
      {
        const res = await fetch(this.$store.state.hostname + '/OquvMarkazCompany/' + this.id);
    ...
    save_data :  async function(){
        if(this.$v.$invalid)
          ...
          const requestOptions = {
            ...
            try{
              this.loading = true;
              const response = await fetch(this.$store.state.hostname + "/OquvMarkazCompany", requestOptions);

      ...
      <router-link to="/probniyFans">
         <h5 class="m-0 ml-3 d-flex" style="padding: 14px 0px">
           <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-chevron-left mr-2" width="24" height="24" viewBox="0 0 24 24" stroke-width="2.5" stroke="#007BFF" fill="none" stroke-linecap="round" stroke-linejoin="round">
            ...
            {{$t('Add_probniyFans')}}</h5>
      </router-link>
     
    </div>
    <div class="probniyFans_add px-5 pt-4">
      <form @submit.prevent="submit">
        <div class="row_w">
          ...
          <div class="col_5 px-2">
            <input-icon  v-model="name" valid validate error="wrong" success="right" icon="bookmark"/>
        ...
        <div class="row_w mt-4">
          <div class="col_3">
            <p class="p-0 m-0 mt-2" style="font-size: 14px;">{{$t('begin_time')}}</p>
          </div>
        ...
        <div class="row_w mt-4">
          <div class="col_3">
            <p class="p-0 m-0 mt-2" style="font-size: 14px;">{{$t('end_time')}}</p>
          </div>
          <div class="col_2 px-2">
        ...
        <div class="row_w mt-4">
          <div class="col_3">
            <p class="p-0 m-0 mt-2" style="font-size: 14px;">{{$t('intermade_date')}}</p>
          </div>
          <div class="col_5 px-2">
            <input-icon  v-model="intermade_date" type="date"  validate error="wrong" success="right" />
            <!-- <small class="invalid-text mt-1" style="margin-left:5px; "  v-if="$v.name.$dirty && !$v.name.required" >
              {{$t('name_invalid_text')}}
            </small> -->
            <!-- <small class="p-0" style="margin-left:5px; font-size: 12px; color: gray;"  v-else >
...
export default {
  ...
  validations: {
    ...
    async created()
    {
      ...
      if(this.id > 0)
      {
        const res = await fetch(this.$store.state.hostname + '/OquvMarkazProbniyDarslar/' + this.id);
        const data = await res.json();
        // console.log('this is by id')
        // console.log(data)
        this.name = data.name;
        this.begin_time = data.begin_time.slice(0,5);
        this.end_time = data.end_time.slice(0,5);
        this.intermade_date = data.intermade_date.slice(0,10);
        ...
        if(this.$v.$invalid)
          ...
          const requestOptions = {
              ...
              body: JSON.stringify({
                ...
                "intermade_date": this.intermade_date,
                "id" : this.id,
              })
            };
            try{
              this.loading = true;
              const response = await fetch(this.$store.state.hostname + "/OquvMarkazProbniyDarslar", requestOptions);
    ...
    async submit_continue()
      {
        ...
        {
          if(this.id != 0){
            this.$router.push('/probniyFans_add/0')
            this.cls_wnd();
          }
          else{
            this.cls_wnd();

...
export default {
  ...
  validations: {
    ...
    async created()
    {
      if(this.id > 0)
      {
        const res = await fetch(this.$store.state.hostname + '/OquvMarkazDepartment/' + this.id);
  ...
  methods:{
    ...
    save_data :  async function(){
        ...
        const requestOptions = {
            ...
            body: JSON.stringify({
              "name" : this.name,
              "oquvMarkazCompanyid": this.comp_id,
              "id": this.id
            })
          };
          try{
            this.loading = true;
            const response = await fetch(this.$store.state.hostname + "/OquvMarkazDepartment", requestOptions);

...
export default {
  ...
  validations: {
    ...
    async created()
    {
      ...
      if(this.id > 0)
      {
        const res = await fetch(this.$store.state.hostname + '/OquvMarkazClient/deleteOquvMarkazClientType?id=' + this.id);
    ...
    save_data :  async function(){
        if(this.$v.$invalid)
          ...
          const requestOptions = {
            ...
            try{
              this.loading = true;
              const response = await fetch(this.$store.state.hostname + "/OquvMarkazClient/addOquvMarkazClientType", requestOptions);

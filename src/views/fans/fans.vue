...
export default {
  ...
  computed: mapGetters(['allFans']),
  methods: {
    ...mapActions(['fetchFans', ]),
    ...mapMutations(['fans_delete_row']),
  
      for_edit(edit_data)
      {
        this.$router.push("/fans_add/" + edit_data.id);
      },
      async for_delete(del_data,index)
      {
         const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/OquvMarkazFanlar/" + del_data.id, requestOptions);

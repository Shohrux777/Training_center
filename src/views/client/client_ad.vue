<template>
  <div class="py-2 px-2">
    ...
    <div class="client_add px-5 pt-4" style="position:relative;">
      <div class="free_day ">
        <div class="px-1">
          <h6 class="text-primary" style="font-size: 13.5px;">Выходные</h6>
        </div>
        <div class="week_day_item d-flex align-items-center py-2 px-1 " v-for="(item,i) in free_day" :key="i" @click="choose_freeDay(i)">
        ...
        <div class="px-1 mt-3">
          <h6 class="text-primary " style="font-size: 13.5px;">Свободное время</h6>
          <input-icon type="time" style="width: 70%; height: 35px;"  valid v-model="free_time" icon=""/>
        </div>
      </div>
      <form @submit.prevent="submit">
        <div class=" row_w">
          <div class="col_3">
            <p class="p-0 m-0 mt-2" style="font-size: 13.5px;">{{$t('namef')}}</p>
          </div>
          <div class="col_6 px-2">
        ...
        <div class="row_w mt-4">
          <div class="col_3">
            <p class="p-0 m-0 mt-2" style="font-size: 13.5px;">{{$t('namei')}}</p>
          </div>
          <div class="col_6 px-2">
        ...
        <div class="mt-4 row_w">
          <div class="col_3">
            <p class="p-0 m-0 mt-2" style="font-size: 13.5px;">{{$t('phoneNumber')}}</p>
          </div>
          <div class="col_6 px-2">
            <input-phone  v-model="phoneNumber"  valid error="wrong" success="right" />
        ...
        <div class="mt-4 row_w">
          <div class="col_3">
            <p class="p-0 m-0 mt-2" style="font-size: 13.5px;">{{$t('phoneNumberTeleg')}}</p>
          </div>
          <div class="col_6 px-2">
            <input-phone  v-model="phoneNumber1"   error="wrong" success="right"  fab/>
            <!-- <small class="invalid-text pt-4" style="margin-left:5px; "  v-if="$v.phoneNumber.$dirty && !$v.phoneNumber.required" >
              {{$t('name_invalid_text')}}
            </small>
            <small class="p-0" style="margin-left:5px; font-size: 12px; color: gray;"  v-else >
        ...
        <div class="mt-4 row_w">
          <div class="col_3">
            <p class="p-0 m-0 mt-2" style="font-size: 13.5px;">{{$t('phoneNumberFather')}}</p>
          </div>
          <div class="col_6 px-2">
            <input-phone  v-model="phoneNumber2"   error="wrong" success="right" />
            <!-- <small class="invalid-text pt-4" style="margin-left:5px; "  v-if="$v.phoneNumber.$dirty && !$v.phoneNumber.required" >
              {{$t('name_invalid_text')}}
            </small>
            <small class="p-0" style="margin-left:5px; font-size: 12px; color: gray;"  v-else >
        ...
        <div class="mt-4 row_w">
          <div class="col_3">
            <p class="p-0 m-0 mt-2" style="font-size: 13.5px;">{{$t('born_date')}}</p>
          </div>
          <div class="col_6 px-2">
        ...
        <div class="mt-4 row_w">
          <div class="col_3">
            <p class="p-0 m-0 mt-2" style="font-size: 13.5px;">{{$t('contragent')}}</p>
          </div>
          <div class="col_6 px-2">
            <ErpSelectIcon
        ...
        <div class="mt-4 row_w">
          <div class="col_3">
            <p class="p-0 m-0 mt-2" style="font-size: 13.5px;">{{$t('level')}}</p>
          </div>
          <div class="col_6 px-2">
            <ErpSelectIcon
        ...
        <!-- <div class="mt-4 row_w">
          <div class="col_3">
            <p class="p-0 m-0 mt-2" style="font-size: 13.5px;">{{$t('picture')}}</p>
          </div>
...
export default {
  ...
  validations: {
    ...
    async created()
    {
      ...
      if(this.id > 0)
      {
        ...
        this.phoneNumber1 = data.phone_number;
        this.phoneNumber2 = data.phone_number;
  ...
  methods:{
    ...mapActions(['fetch_district_t', 'fetchClient', 'fetch_contragent_t','fetchGroups', 'fetchLevel', 'fetchClientListByDate']),
          ...
          const requestOptions = {
            ...
            try{
              ...
              if(response.status == 201 || response.status == 200)
              {
                ...
                await this.addClientToGroup(dataJson.id);
                this.fetchClient();
                await this.update_client_list();
                return true;
              }
              else{
    ...
    async submit()
      ...
      async addClientToGroup(client_id){
        const requestOptions = {
          method : "POST",
          headers: { "Content-Type" : "application/json" },
          body: JSON.stringify({
            "oquvMarkazClientid" : client_id,
            'oquvMarkazPupilGroupsid': this.groups_id,
            "id" : 0,
          })
        };
        try{
          ...
          {
            this.$refs.message.success('Added_successfully')
            this.fetchGroupsClientList(this.groups_id)
            return true;
          }
          else{
            this.modal_info = data;

    ...
    <div class="product_add px-5 pt-4">
      <form @submit.prevent="submit">
        ...
        <div class="row_w mt-4">
          ...
          <div class="col_5 px-2">
            <input-icon  v-model="price"    icon="dollar-sign"/>
            <!-- <small class="p-0" style="margin-left:5px; font-size: 12px; color: gray;"  v-else >
              {{$t('write_district_name_full')}}
            </small> -->
          </div>
...
export default {
  ...
  validations: {
    ...
    async created()
    {
      ...
      if(this.id > 0)
      {
        const res = await fetch(this.$store.state.hostname + '/OquvMarkazProduct/' + this.id);
    ...
    save_data :  async function(){
        if(this.$v.$invalid)
          ...
          const requestOptions = {
            ...
            try{
              this.loading = true;
              const response = await fetch(this.$store.state.hostname + "/OquvMarkazProduct", requestOptions);

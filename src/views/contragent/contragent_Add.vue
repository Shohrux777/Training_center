      ...
      <form @submit.prevent="submit">
        <div class="row_w">
          ...
          <div class="col_5 px-2">
            <input-icon  v-model="name"  validate error="wrong" success="right" icon="icons"/>
        ...
        <div class="row_w mt-4">
          <div class="col_3">
            <p class="p-0 m-0 mt-2" style="font-size: 14px;">{{$t('icons')}}</p>
          </div>
          <div class="col_5 px-2 ">
            
            <div class="d-flex justify-content-center align-items-center">
              <div v-for="(item, i) in icons" :key="i" @click="GetIcon(item)">
                <mdb-icon  style="font-size: 30px;" class="ml-3" fab :icon="item.name" :style="{'color': item.color}"  />
              </div>
            </div>
            <div class="d-flex justify-content-center align-items-center">
              <div >
                <mdb-icon  style="font-size: 30px;" class="ml-3 mt-3" fab :icon="icon_base_64_str" :style="{'color': note}"  />
              </div>
            </div>
          </div>
        </div> 
...
export default {
  ...
  data(){
    return{
      ...
      icons: [
        {name: 'instagram', color: '#D8407C'},
        {name: 'facebook', color: 'blue'},
        {name: 'twitter', color: 'cyan'},
        {name: 'telegram', color: '#0C82BD'},
        {name: 'youtube', color: 'red'},
        {name: 'internet-explorer', color: 'cyan'},
  ...
  validations: {
    ...
    async created()
    {
      ...
      if(this.id > 0)
      {
        const res = await fetch(this.$store.state.hostname + '/OquvMarkazClient/getOquvMarkazContragentById?id=' + this.id);
        const data = await res.json();
        console.log('this is by id')
        console.log(data)
        this.name = data.name,
        this.note = data.note,
        this.icon_base_64_str = data.icon_base_64_str
      }
        ...
        if(this.$v.$invalid)
          ...
          const requestOptions = {
              ...
              body: JSON.stringify({
                "name" : this.name,
                "note": this.note,
                "icon_base_64_str": this.icon_base_64_str,
                "id" : this.id,
              })
            };
            try{
              this.loading = true;
              const response = await fetch(this.$store.state.hostname + "/OquvMarkazClient/addOquvMarkazContragent", requestOptions);

...
export default {
  ...
  methods: {
    ...
    ...mapMutations(['groups_delete_row']),
  
      for_edit(edit_data)
      {
        this.$router.push("/groups_add/" + edit_data.id);
      },
      async for_delete(del_data,index)
      {
         const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/OquvMarkazPupilGroups/" + del_data.id, requestOptions);
      ...
      async startFunc(option){
        console.log('keldi')
        console.log(option)
        try{
          this.loading = true;
          const response = await fetch(this.$store.state.hostname + '/OquvMarkazPupilGroups/getOpenOrCloseGroupByidAndStatus?group_id='+ option.id +'&status=true');
